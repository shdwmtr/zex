...
impl Window {
    ...
    /// Starts a native OS drag-and-drop session carrying the given file paths,
    /// so the dragged item can be dropped onto other applications (Wayland only for now).
    pub fn start_file_drag(&self, paths: Vec<PathBuf>, icon: Option<(Size<DevicePixels>, Vec<u8>)>) {
        self.platform_window.start_drag(paths, icon)
    }
