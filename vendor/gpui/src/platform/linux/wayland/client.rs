...
use crate::{
    SharedString,
    platform::linux::{
        LinuxClient, encode_file_uri_list, get_xkb_compose_state, is_within_click_distance,
        open_uri_internal, read_fd, reveal_path_internal,
        wayland::{
            clipboard::{
                Clipboard, DataOffer, FILE_LIST_MIME_TYPE, GNOME_FILE_LIST_MIME_TYPE,
                TEXT_MIME_TYPES,
            },
            cursor::Cursor,
...
pub(crate) struct WaylandClientState {
    ...
    common: LinuxCommon,
    outgoing_drag: Option<OutgoingDrag>,
}

struct OutgoingDragIcon {
...
impl Drop for OutgoingDragIcon {
    fn drop(&mut self) {
        self.buffer.destroy();
...
struct OutgoingDrag {
    ...
    uri_list: String,
    _icon: Option<OutgoingDragIcon>,
}

impl Drop for OutgoingDrag {
    fn drop(&mut self) {
...
impl ShmBacking {
    fn new(len: usize) -> Option<(Self, OwnedFd)> {
        let name = c"gpui-drag-icon";
        let raw_fd = unsafe {
        ...
        Some((Self { ptr, len }, fd))
    }

    /// # Safety
    /// The caller must not write past `len` bytes, matching the size this backing was created with.
...
fn create_drag_icon(
    ...
    let (mut shm_backing, fd) = ShmBacking::new(len)?;
    {
        let dst = unsafe { shm_backing.as_mut_slice() };
    ...
    let viewport = viewporter.map(|viewporter| viewporter.get_viewport(&surface, qh, ()));
    if let Some(viewport) = &viewport {
        let scale = if scale > 0.0 { scale } else { 1.0 };
        let logical_width = ((width as f32) / scale).round().max(1.0) as i32;
        let logical_height = ((height as f32) / scale).round().max(1.0) as i32;
        viewport.set_destination(logical_width, logical_height);
    }

    surface.commit();

    Some(OutgoingDragIcon {
        surface,
        viewport,
        buffer,
        _shm: shm_backing,
...
impl WaylandClientStatePtr {
    ...
    pub fn start_file_drag(
        &self,
        paths: Vec<PathBuf>,
        icon: Option<(Size<DevicePixels>, Vec<u8>)>,
        scale: f32,
        origin: &wl_surface::WlSurface,
        serial: u32,
    ) {
        eprintln!(
            "[wayland-drag] start_file_drag called, {} paths, serial={serial}",
            paths.len()
        );
        if paths.is_empty() {
            eprintln!("[wayland-drag] aborting: no paths");
            return;
        }
        ...
        if state.outgoing_drag.is_some() {
            eprintln!("[wayland-drag] aborting: a drag is already outgoing");
            return;
        }
        ...
        let (Some(data_device_manager), Some(data_device)) = (
            ...
            eprintln!(
                "[wayland-drag] aborting: data_device_manager={} data_device={}",
                state.globals.data_device_manager.is_some(),
                state.data_device.is_some()
            );
            return;
        };

        let icon = icon.and_then(|(size, pixels)| {
            create_drag_icon(
                &state.globals.shm,
                &state.globals.compositor,
                state.globals.viewporter.as_ref(),
                &state.globals.qh,
                size,
                scale,
                &pixels,
        ...
        eprintln!("[wayland-drag] icon surface created: {}", icon.is_some());
        ...
        eprintln!("[wayland-drag] wl_data_device.start_drag issued");

        state.outgoing_drag = Some(OutgoingDrag {
            data_source,
            uri_list: encode_file_uri_list(&paths),
            _icon: icon,
        });
    }
}
...
impl Dispatch<wl_data_source::WlDataSource, ()> for WaylandClientStatePtr {
    fn event(
        ...
        let is_outgoing_drag = state
            .outgoing_drag
            .as_ref()
            .is_some_and(|drag| drag.data_source.id() == data_source.id());

        if is_outgoing_drag {
            eprintln!("[wayland-drag] data source event: {event:?}");
        }

        match event {
            wl_data_source::Event::Send { mime_type, fd } => {
                if is_outgoing_drag {
                    if let Some(drag) = state.outgoing_drag.as_ref() {
                        if mime_type == FILE_LIST_MIME_TYPE {
                            state.clipboard.send_bytes(fd, drag.uri_list.clone().into_bytes());
                        }
                    }
                } else {
