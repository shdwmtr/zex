...
use crate::{
    ...
    platform::linux::platform::{
        decode_file_uri_list, decode_gnome_copied_files, encode_file_uri_list,
        encode_gnome_copied_files, read_fd,
    },
};
...
/// Used by Nautilus and other GNOME/GTK file managers to distinguish a cut
/// from a copy, which plain `text/uri-list` can't express.
pub(crate) const GNOME_FILE_LIST_MIME_TYPE: &str = "x-special/gnome-copied-files";
...
impl<T: ReceiveData> DataOffer<T> {
    ...
    fn read_paths(&self, connection: &Connection) -> Option<ClipboardItem> {
        if self.has_mime_type(GNOME_FILE_LIST_MIME_TYPE) {
            let bytes = self.read_bytes(connection, GNOME_FILE_LIST_MIME_TYPE)?;
            if let Some(paths) = decode_gnome_copied_files(&bytes) {
                return Some(ClipboardItem {
                    entries: vec![ClipboardEntry::Paths(paths)],
                });
            }
        }
        ...
        if self.has_mime_type(FILE_LIST_MIME_TYPE) {
            ...
            let paths = decode_file_uri_list(&text);
            if !paths.is_empty() {
                return Some(ClipboardItem {
                    entries: vec![ClipboardEntry::Paths(crate::ClipboardPaths {
                        paths,
                        op: crate::ClipboardPathsOp::Copy,
                    })],
                });
            }
        }
...
impl Clipboard {
    ...
    pub fn send(&self, mime_type: String, fd: OwnedFd) {
        Self::send_contents(self.contents.as_ref(), &mime_type, fd, |fd, bytes| {
    ...
    pub fn send_primary(&self, mime_type: String, fd: OwnedFd) {
        Self::send_contents(self.primary_contents.as_ref(), &mime_type, fd, |fd, bytes| {
            self.send_internal(fd, bytes)
        });
    }
    ...
    fn send_contents(
        ...
        mime_type: &str,
        fd: OwnedFd,
        write: impl FnOnce(OwnedFd, Vec<u8>),
    ) {
        if mime_type == FILE_LIST_MIME_TYPE
            && let Some(paths) = contents.and_then(|contents| contents.paths())
        {
            write(fd, encode_file_uri_list(&paths.paths).into_bytes());
            return;
        }

        if mime_type == GNOME_FILE_LIST_MIME_TYPE
            && let Some(paths) = contents.and_then(|contents| contents.paths())
        {
            write(fd, encode_gnome_copied_files(paths));
            return;
        }

        if let Some(text) = contents.and_then(|contents| contents.text()) {
            write(fd, text.as_bytes().to_owned());
        }
    }
