...
/// Percent-encodes `path` into a `file://` URI, as used by the `text/uri-list`
/// and `x-special/gnome-copied-files` clipboard/DnD mime types.
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(crate) fn encode_file_uri(path: &Path) -> String {
...
/// Encodes `paths` as a `text/uri-list` body (CRLF-terminated per RFC 2483).
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(crate) fn encode_file_uri_list(paths: &[PathBuf]) -> String {
    let mut uri_list = String::new();
    for path in paths {
        uri_list.push_str(&encode_file_uri(path));
        uri_list.push_str("\r\n");
    }
    uri_list
}

/// Parses a `text/uri-list` body into file paths, skipping comment lines
/// (starting with `#`, per RFC 2483) and any URIs that aren't `file://`.
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(crate) fn decode_file_uri_list(text: &str) -> Vec<PathBuf> {
    text.lines()
        .map(str::trim)
        .filter(|line| !line.is_empty() && !line.starts_with('#'))
        .filter_map(|line| http_client::Url::parse(line).ok())
        .filter_map(|url| url.to_file_path().ok())
        .collect()
}

/// Encodes `paths` as an `x-special/gnome-copied-files` body, which Nautilus
/// and other GNOME/GTK file managers use to distinguish cut from copy.
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(crate) fn encode_gnome_copied_files(paths: &crate::ClipboardPaths) -> Vec<u8> {
    let mut body = match paths.op {
        crate::ClipboardPathsOp::Copy => String::from("copy"),
        crate::ClipboardPathsOp::Cut => String::from("cut"),
    };
    for path in &paths.paths {
        body.push('\n');
        body.push_str(&encode_file_uri(path));
    }
    body.into_bytes()
}

/// Parses an `x-special/gnome-copied-files` body into a [`crate::ClipboardPaths`].
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(crate) fn decode_gnome_copied_files(bytes: &[u8]) -> Option<crate::ClipboardPaths> {
    let text = std::str::from_utf8(bytes).ok()?;
    let mut lines = text.lines();
    let op = match lines.next()?.trim() {
        "cut" => crate::ClipboardPathsOp::Cut,
        _ => crate::ClipboardPathsOp::Copy,
    };
    let paths = decode_file_uri_list(lines.collect::<Vec<_>>().join("\n").as_str());
    if paths.is_empty() {
        None
    } else {
        Some(crate::ClipboardPaths { paths, op })
    }
}
