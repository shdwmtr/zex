...
/// Rasterizes SVG assets to pixel buffers.
#[derive(Clone)]
...
/// The target size to rasterize an SVG at.
pub enum SvgSize {
    /// Rasterize to this exact pixel size.
    Size(Size<DevicePixels>),
    /// Rasterize at the SVG's native size multiplied by this factor.
    ScaleFactor(f32),
}

impl SvgRenderer {
    /// Creates a new renderer that loads SVG bytes from the given asset source.
    ...
    /// Rasterizes the given SVG bytes to a pixmap at the requested size.
