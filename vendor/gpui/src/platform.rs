...
pub(crate) trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    fn start_drag(&self, _paths: Vec<PathBuf>, _icon: Option<(Size<DevicePixels>, Vec<u8>)>) {}
...
/// Either a ClipboardString, a ClipboardImage, or a list of file paths
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ClipboardEntry {
    ...
    /// An image entry
    Image(Image),
    /// A list of file paths, as found on the clipboard when a file manager
    /// copies or cuts files (e.g. Nautilus, Dolphin, Explorer, Finder)
    Paths(ClipboardPaths),
}

/// Whether a [`ClipboardPaths`] entry represents a copy or a cut/move,
/// mirroring how file managers represent cut-vs-copy on the clipboard.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ClipboardPathsOp {
    /// The files should be copied to the destination.
    Copy,
    /// The files should be moved to the destination.
    Cut,
}

/// A list of file paths on the clipboard, with an associated copy/cut intent.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClipboardPaths {
    /// The file paths on the clipboard.
    pub paths: Vec<PathBuf>,
    /// Whether these paths were cut or copied.
    pub op: ClipboardPathsOp,
}
...
impl ClipboardItem {
    ...
    /// Create a new ClipboardItem::Paths with the given file paths
    pub fn new_paths(paths: ClipboardPaths) -> Self {
        Self {
            entries: vec![ClipboardEntry::Paths(paths)],
        }
    }

    /// If this item contains a `ClipboardEntry::Paths`, returns it.
    pub fn paths(&self) -> Option<&ClipboardPaths> {
        self.entries.iter().find_map(|entry| match entry {
            ClipboardEntry::Paths(paths) => Some(paths),
            _ => None,
        })
    }
