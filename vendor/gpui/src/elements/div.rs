...
use crate::{
    ...
    DevicePixels, DispatchPhase, Display, Element, ElementId, Entity, FocusHandle, Global,
...
impl Interactivity {
    ...
    /// On drag initiation, this callback starts a native OS drag-and-drop session carrying the
    /// returned file paths (with an optional preview icon), so the dragged item can be dropped
    /// onto other applications. Return `None` to skip starting an OS drag for this gesture.
    /// Mutually exclusive with [`Self::on_drag`] for a given element.
    pub fn on_drag_out(
        &mut self,
        paths: impl Fn(&mut Window, &mut App) -> Option<(Vec<PathBuf>, Option<(Size<DevicePixels>, Vec<u8>)>)>
        + 'static,
    ) where
        Self: Sized,
    {
        debug_assert!(
            self.drag_out_listener.is_none(),
            "calling on_drag_out more than once on the same element is not supported"
        );
        self.drag_out_listener = Some(Box::new(paths));
    }
...
pub trait StatefulInteractiveElement: InteractiveElement {
    ...
    /// The fluent API equivalent to [`Interactivity::on_drag_out`]
    fn on_drag_out(
        mut self,
        paths: impl Fn(&mut Window, &mut App) -> Option<(Vec<PathBuf>, Option<(Size<DevicePixels>, Vec<u8>)>)>
        + 'static,
    ...
    where
        Self: Sized,
    {
        self.interactivity().on_drag_out(paths);
        self
    }
...
pub(crate) type DragOutListener = Box<
    dyn Fn(&mut Window, &mut App) -> Option<(Vec<PathBuf>, Option<(Size<DevicePixels>, Vec<u8>)>)>
        + 'static,
>;
...
pub struct Interactivity {
    ...
    pub(crate) drag_out_listener: Option<DragOutListener>,
...
impl Interactivity {
    ...
    fn should_insert_hitbox(&self, style: &Style, window: &Window, cx: &App) -> bool {
        self.hitbox_behavior != HitboxBehavior::Normal
            ...
            || self.drag_out_listener.is_some()
    ...
    fn paint_mouse_listeners(
        ...
        let mut drag_out_listener = mem::take(&mut self.drag_out_listener);
        ...
        if let Some(element_state) = element_state {
            if !click_listeners.is_empty() || drag_listener.is_some() || drag_out_listener.is_some() {
                            ...
                            && (event.position - mouse_down.position).magnitude() > DRAG_THRESHOLD
                        {
                            if let Some((drag_value, drag_listener)) = drag_listener.take() {
                                *clicked_state.borrow_mut() = ElementClickedState::default();
                                ...
                                window.refresh();
                                cx.stop_propagation();
                            } else if let Some(drag_out) = drag_out_listener.take()
                                && let Some((paths, icon)) = drag_out(window, cx)
                            {
                                window.start_file_drag(paths, icon);
                                pending_mouse_down.take();
                                window.refresh();
                                cx.stop_propagation();
                            }
